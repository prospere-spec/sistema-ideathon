...
describe("phase dates", () => {
  it("accepts valid ISO date times and preserves empty values", () => {
    expect(parsePhaseDates({ startsAt: "2026-09-11T09:00:00.000Z", endsAt: "" })).toMatchObject({ ok: true, data: { startsAt: new Date("2026-09-11T09:00:00.000Z"), endsAt: null } });
  });

  it("rejects invalid dates and inverted date ranges", () => {
    expect(parsePhaseDates({ startsAt: "not-a-date" })).toMatchObject({ ok: false, error: expect.any(String) });
