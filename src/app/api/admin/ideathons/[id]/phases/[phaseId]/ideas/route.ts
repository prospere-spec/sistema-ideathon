...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const [ideaRows, roomRows] = await Promise.all([
      db.select({ id: ideas.id, name: ideas.name, category: ideas.category, teamName: teams.name, phaseIdeaId: phaseIdeas.id, roomId: phaseIdeas.roomId, roomName: rooms.name, presentationOrder: phaseIdeas.presentationOrder, participationStatus: phaseIdeas.status }).from(ideas).innerJoin(teams, eq(teams.id, ideas.teamId)).leftJoin(phaseIdeas, and(eq(phaseIdeas.ideaId, ideas.id), eq(phaseIdeas.phaseId, phaseId))).leftJoin(rooms, eq(rooms.id, phaseIdeas.roomId)).where(and(eq(ideas.ideathonId, id), eq(ideas.status, "ACTIVE"))).orderBy(asc(ideas.createdAt)),
    ...
    console.error("Failed to load phase ideas", error);
    return NextResponse.json({ error: "Não foi possível carregar as ideias desta fase." }, { status: 500 });
  }
}
