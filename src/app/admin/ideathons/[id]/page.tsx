    ...
    <AppShell activeSection="overview">
      <div className="mx-auto w-full max-w-container space-y-6 px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
        <section className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
          <div>
            <div className="mb-4 flex items-center gap-3 text-xs font-semibold text-ink-muted"><a href="/admin" className="hover:text-ink">Events</a><ChevronRight className="size-3.5" /><span>FinTech Innovate 2024</span></div>
          ...
          <div className="flex flex-wrap items-center gap-3"><a href="/public/fintech-innovate-2024" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-ink-muted bg-white px-4 text-sm font-semibold transition-colors hover:bg-surface-low"><Globe className="size-4" />View Public Page</a><a href="/admin/ideathons/fintech-2024/configuracao" className="inline-flex min-h-11 items-center gap-2 rounded-md bg-lime px-4 text-sm font-bold text-lime-foreground transition-colors hover:bg-lime/85"><Pencil className="size-4" />Edit Event</a></div>
        </section>
        ...
        <div className="grid grid-cols-1 items-start gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(320px,0.98fr)]">
          <div className="space-y-6">
            <nav className="flex gap-7 overflow-x-auto border-b border-outline/60" aria-label="Detalhes do ideathon">
              {tabs.map((tab) => <button type="button" key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap border-b-2 px-1 pb-4 font-serif text-base font-bold transition-colors ${activeTab === tab ? "border-primary text-ink" : "border-transparent text-ink-muted hover:text-ink"}`}>{tab}</button>)}
            </nav>
            {activeTab !== "Overview" ? <PlaceholderTab tab={activeTab} /> : <>
              ...
              <section className="rounded-lg border border-black/[0.04] bg-white p-6 shadow-card sm:p-8" aria-labelledby="criteria-title"><div className="flex items-center justify-between gap-4"><h2 id="criteria-title" className="font-serif text-2xl font-bold text-ink">Evaluation Criteria</h2><a href="/admin/ideathons/fintech-2024/configuracao" className="inline-flex items-center gap-2 text-sm font-serif text-ink-muted hover:text-ink"><Settings className="size-4" />Configure</a></div><div className="mt-7 grid grid-cols-1 gap-4 sm:grid-cols-2">{eventCriteria.map((criterion) => <div key={criterion.name} className="flex items-center justify-between gap-4 rounded-md bg-surface-high px-5 py-5"><span className="font-serif text-lg text-ink">{criterion.name}</span><span className="shrink-0 rounded bg-white px-3 py-1.5 font-serif text-sm font-bold text-ink shadow-sm">{criterion.weight}</span></div>)}</div></section>

              <section className="rounded-lg border border-black/[0.04] bg-white p-6 shadow-card sm:p-8" aria-labelledby="top-teams-title"><div className="flex items-center justify-between gap-4"><h2 id="top-teams-title" className="font-serif text-2xl font-bold text-ink">Top Teams (Provisional)</h2><a href="/admin/ideathons/fintech-2024/resultados" className="font-serif text-sm text-ink-muted hover:text-ink">View All</a></div><div className="mt-6 divide-y divide-outline/30">{topTeams.map((team) => <div key={team.name} className="flex items-center gap-4 py-5 first:pt-0 last:pb-0"><span className="w-6 font-serif text-base text-ink">{team.rank}</span><span className={`flex size-12 shrink-0 items-center justify-center rounded-md font-serif text-sm font-bold ${team.featured ? "bg-primary text-lime" : "bg-surface-container text-ink"}`}>{team.initials}</span><div className="min-w-0 flex-1"><p className="font-serif text-lg text-ink">{team.name}</p><p className="font-serif text-base text-ink-muted">{team.description}</p></div><span className="shrink-0 rounded-full bg-surface-container px-3 py-2 font-serif text-sm font-bold text-ink">{team.score}</span></div>)}</div></section>
            </>}
          </div>
