...
import { Clipboard, Copy, Filter, LoaderCircle, Plus, RefreshCw, Trash2, UserRound, X } from "lucide-react";
...
export default function UsersPage() {
  ...
  async function deleteUser(user: PlatformUser) {
    if (!window.confirm(`Excluir ${user.name}? O acesso será removido e o histórico será preservado de forma anônima.`)) return;
    setSaving(true);
    setError("");
    try {
      ...
      if (temporaryPasswordUser?.id === user.id) setTemporaryPasswordUser(null);
      setNotice("Usuário excluído. Os registros históricos foram preservados.");
    } catch (deleteError) {
  ...
  return (
    <AppShell navigation="management" activeSection="users" headerAction={<Button type="button" onClick={() => setModalOpen(true)} className="min-h-9 rounded-full px-4 text-xs"><Plus className="size-3.5" />Novo usuário</Button>}>
      <div className="mx-auto w-full max-w-container space-y-6 px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
        <section className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
          <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-lime-deep">Administração</p><h1 className="mt-2 text-3xl font-bold tracking-[-0.055em] text-ink sm:text-4xl">Gestão de usuários</h1><p className="mt-2 text-base text-ink-muted">Gerencie acessos, permissões e status persistidos na plataforma.</p></div>
          <Button type="button" onClick={() => setModalOpen(true)}><Plus className="size-4" />Adicionar usuário</Button>
        </section>
        ...
        <section className="flex flex-col gap-3 rounded-lg border border-outline/45 bg-white p-4 shadow-card sm:flex-row sm:items-end">
          <button type="button" onClick={() => setFiltersOpen((open) => !open)} className="inline-flex min-h-10 items-center gap-2 self-start rounded-md border border-outline px-4 text-sm font-semibold hover:bg-surface-low" aria-expanded={filtersOpen}><Filter className="size-4" />Filtros</button>
          {filtersOpen ? <><Input id="user-search" label="Buscar usuário" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Nome ou e-mail" className="min-h-10" /><Select id="user-role" label="Perfil" value={roleFilter} onChange={(event) => setRoleFilter(event.target.value as "all" | UserRole)} className="min-h-10"><option value="all">Todos os perfis</option><option value="ADMIN">Administradores</option><option value="EVALUATOR">Avaliadores</option></Select><Select id="user-status" label="Status" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "all" | UserStatus)} className="min-h-10"><option value="all">Todos os status</option><option value="ACTIVE">Ativos</option><option value="INACTIVE">Inativos</option></Select></> : <p className="text-sm text-ink-muted">{filteredUsers.length} usuário(s) encontrado(s).</p>}
        </section>

        <section className="overflow-hidden rounded-lg border border-outline/45 bg-white shadow-card">
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] border-collapse text-left"><thead><tr className="bg-surface-low"><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Usuário</th><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Perfil</th><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Status</th><th className="px-6 py-4 text-right text-xs font-semibold text-ink-muted">Ações</th></tr></thead><tbody className="divide-y divide-outline/30">{loading ? <tr><td colSpan={4} className="px-6 py-12 text-center text-sm text-ink-muted"><LoaderCircle className="mx-auto size-5 animate-spin" />Carregando usuários...</td></tr> : filteredUsers.map((user) => <tr key={user.id} className="hover:bg-surface-low/40"><td className="px-6 py-4"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-full bg-primary text-xs font-bold text-lime">{user.name.split(/\s+/).slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span><div><p className="text-sm font-bold text-ink">{user.name}</p><p className="text-xs text-ink-muted">{user.email}</p></div></div></td><td className="px-6 py-4 text-sm text-ink">{roleLabels[user.role]}</td><td className="px-6 py-4"><Badge tone={user.status === "ACTIVE" ? "lime" : "neutral"}>{statusLabels[user.status]}</Badge></td><td className="px-6 py-4 text-right"><div className="flex justify-end gap-2"><Button type="button" variant="secondary" disabled={saving} onClick={() => void resetPassword(user)}><RefreshCw className="size-4" />Senha temporária</Button><Button type="button" variant="ghost" disabled={saving} onClick={() => void toggleStatus(user)}>{user.status === "ACTIVE" ? "Desativar" : "Ativar"}</Button><Button type="button" variant="danger" disabled={saving} onClick={() => void deleteUser(user)}><Trash2 className="size-4" />Excluir</Button></div></td></tr>)}</tbody></table></div>
          {!loading && !filteredUsers.length ? <div className="px-6 py-12 text-center text-sm text-ink-muted"><UserRound className="mx-auto mb-2 size-5" />Nenhum usuário encontrado.</div> : null}
        </section>
      </div>
