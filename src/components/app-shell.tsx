      ...
      <header className={`fixed left-0 right-0 top-0 z-30 h-[72px] border-b backdrop-blur-md ${darkHeader ? "border-white/10 bg-black/95 text-white" : "border-outline/30 bg-surface/90"}`}>
        <div className="flex h-full items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-4">
            <Link href={homeHref} className={`hidden shrink-0 items-center gap-2 text-[22px] font-bold tracking-[-0.06em] md:flex ${darkHeader ? "text-lime" : "text-ink"}`} aria-label="Revvolução, página inicial"><Zap className="size-5 fill-lime-deep text-lime-deep" strokeWidth={2.5} />Revvolução</Link>
            <form action="/admin/ideathons" method="get" className="relative hidden sm:block"><Search className={`pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 ${darkHeader ? "text-white/60" : "text-ink-muted/60"}`} aria-hidden="true" /><label htmlFor="global-search" className="sr-only">Buscar ideathons</label><input id="global-search" name="q" className="h-10 w-64 rounded-full border-0 bg-surface-container pl-10 pr-4 text-sm text-ink placeholder:text-ink-muted/60 transition-colors focus:bg-white focus:outline-none focus:ring-2 focus:ring-lime/50 lg:w-72" placeholder="Buscar ideathons..." /></form>
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            ...
            <div className="relative"><button type="button" onClick={() => setNotificationsOpen((open) => !open)} className={`relative rounded-md p-2 ${darkHeader ? "text-white/80 hover:bg-white/10 hover:text-white" : "text-ink-muted hover:bg-surface-container hover:text-ink"}`} aria-label="Notificações" aria-expanded={notificationsOpen} aria-controls="notifications-popover"><Bell className="size-5" /><span className={`absolute right-2 top-1.5 size-2 rounded-full bg-danger ring-2 ${darkHeader ? "ring-black" : "ring-surface"}`} /></button>{notificationsOpen ? <div id="notifications-popover" role="status" className="absolute right-0 top-12 w-72 rounded-lg border border-outline/40 bg-white p-4 text-ink shadow-popover"><p className="text-sm font-bold">Notificações</p><p className="mt-2 text-xs leading-5 text-ink-muted">Você tem avaliações pendentes para acompanhar.</p><Link href="/admin" onClick={() => setNotificationsOpen(false)} className="mt-3 inline-flex text-xs font-bold text-lime-deep hover:underline">Abrir dashboard</Link></div> : null}</div>
