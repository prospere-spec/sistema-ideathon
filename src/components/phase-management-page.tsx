...
import { Check, Layers3, Pencil, Plus, Save, Trash2 } from "lucide-react";
...
export function PhaseManagementPage({ ideathonId }: { ideathonId: string }) {
  ...
  async function removePhase(phase: Phase) {
    ...
    const payload = await response.json();
    if (!response.ok) { setError(payload.error || "Não foi possível remover a fase."); return; }
  ...
  return <AppShell navigation="management" activeSection="ideathons" darkHeader><div className="mx-auto w-full max-w-container space-y-6 px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
    ...
    <section className="space-y-3" aria-label="Etapas cadastradas">{loading ? <p className="rounded-lg bg-white p-10 text-center text-sm text-ink-muted">Carregando fases...</p> : phases.map((phase) => <article key={phase.id} className="rounded-lg border border-outline/45 bg-white p-5 shadow-card sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-md bg-primary text-lime"><Layers3 className="size-5" /></span><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.12em] text-ink-muted">Etapa {phase.position + 1}</p><h2 className="truncate text-lg font-bold text-ink">{phase.name}</h2></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${phase.status === "LIVE" ? "bg-lime/30 text-lime-deep" : phase.status === "CLOSED" ? "bg-surface-container text-ink-muted" : "bg-indigo/10 text-indigo-deep"}`}>{statusLabels[phase.status]}</span></div><div className="flex flex-wrap gap-2"><Button type="button" variant="secondary" onClick={() => { setEditingId(phase.id); setEditingName(phase.name); setEditingPosition(phase.position); }}><Pencil className="size-4" />Editar</Button>{phase.status === "DRAFT" ? <Button type="button" onClick={() => void updatePhase(phase.id, { status: "READY" })}><Check className="size-4" />Marcar pronta</Button> : null}{phase.status === "READY" ? <Button type="button" onClick={() => void updatePhase(phase.id, { status: "LIVE" })}>Iniciar fase</Button> : null}{phase.status !== "LIVE" && phase.status !== "CLOSED" ? <Button type="button" variant="ghost" onClick={() => void removePhase(phase)} aria-label={`Remover ${phase.name}`}><Trash2 className="size-4" /></Button> : null}</div></div>{editingId === phase.id ? <div className="mt-4 grid grid-cols-[1fr_100px_auto] items-end gap-2 rounded-md bg-surface-low p-3"><label className="text-xs font-bold text-ink-muted">Nome<input value={editingName} onChange={(event) => setEditingName(event.target.value)} className="mt-1 min-h-10 w-full rounded-md border border-outline/60 bg-white px-3 text-sm font-normal text-ink focus:border-lime focus:outline-none" /></label><label className="text-xs font-bold text-ink-muted">Posição<input type="number" min="0" value={editingPosition} onChange={(event) => setEditingPosition(Number(event.target.value))} className="mt-1 min-h-10 w-full rounded-md border border-outline/60 bg-white px-3 text-sm font-normal text-ink focus:border-lime focus:outline-none" /></label><div className="flex gap-1"><Button type="button" disabled={saving} onClick={() => void updatePhase(phase.id, { name: editingName, position: editingPosition })} className="px-3"><Save className="size-4" /><span className="sr-only">Salvar</span></Button><Button type="button" variant="ghost" onClick={() => setEditingId("")} className="px-3">Cancelar</Button></div></div> : null}</article>)}{!loading && !phases.length ? <div className="rounded-lg border border-dashed border-outline bg-white px-6 py-12 text-center text-sm text-ink-muted">Nenhuma etapa cadastrada neste ideathon.</div> : null}</section>
  </div></AppShell>;
}
