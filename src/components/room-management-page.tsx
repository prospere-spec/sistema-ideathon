...
export function RoomManagementPage({ ideathonId }: { ideathonId: string }) {
  ...
  async function saveRoomDetails() {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Não foi possível editar a sala.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function changeStatus(room: Room, status: Room["status"]) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Não foi possível atualizar a sala.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function removeRoom(room: Room) {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Não foi possível remover a sala.");
    } finally {
      setSaving(false);
    }
  }
