...
type PhaseIdea = { id: string; name: string; category: string | null; teamName: string; phaseIdeaId: string | null; roomId: string | null; roomName: string | null; participationStatus: string | null };
...
export function PhaseIdeasPage({ ideathonId, phaseId }: { ideathonId: string; phaseId: string }) {
  ...
  async function assignIdea(idea: PhaseIdea, roomId: string | null) {
    ...
    setIdeas((current) => current.map((item) => item.id === idea.id ? { ...item, phaseIdeaId: payload.data.id, roomId, roomName: room?.name || null } : item));
    setNotice(roomId ? `“${idea.name}” atribuída à ${room?.name}.` : `“${idea.name}” removida da sala.`);
  }

  const editable = phaseStatus !== "LIVE" && phaseStatus !== "CLOSED";
  return (
    <AppShell navigation="management" activeSection="ideathons" darkHeader>
      <div className="mx-auto w-full max-w-container space-y-6 px-4 py-7 sm:px-6 lg:px-8 lg:py-9">
        ...
        <section className="overflow-hidden rounded-lg border border-outline/45 bg-white shadow-card"><div className="overflow-x-auto"><table className="w-full min-w-[720px] border-collapse text-left"><thead><tr className="bg-surface-low"><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Ideia</th><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Equipe</th><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Participação</th><th className="px-6 py-4 text-xs font-semibold text-ink-muted">Sala</th></tr></thead><tbody className="divide-y divide-outline/40">{loading ? <tr><td colSpan={4} className="px-6 py-12 text-center text-sm text-ink-muted">Carregando ideias...</td></tr> : ideas.map((idea) => <tr key={idea.id} className="hover:bg-surface-low/40"><td className="px-6 py-4"><p className="text-sm font-bold text-ink">{idea.name}</p><p className="mt-1 max-w-sm truncate text-xs text-ink-muted">{idea.category || "Sem categoria"}</p></td><td className="px-6 py-4 text-sm text-ink">{idea.teamName}</td><td className="px-6 py-4">{idea.phaseIdeaId ? <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-lime-deep"><Check className="size-4" />Incluída na fase</span> : <Button type="button" variant="secondary" disabled={!editable || savingId === idea.id} onClick={() => void assignIdea(idea, null)} className="min-h-9 px-3 py-1 text-xs"><Layers3 className="size-3.5" />Incluir na fase</Button>}</td><td className="px-6 py-4"><select aria-label={`Sala da ideia ${idea.name}`} disabled={!editable || savingId === idea.id} value={idea.roomId || ""} onChange={(event) => void assignIdea(idea, event.target.value || null)} className="min-h-10 w-full max-w-xs rounded-md border border-outline/70 bg-white px-3 text-sm text-ink focus:border-lime focus:outline-none focus:ring-2 focus:ring-lime/40"><option value="">Sem sala</option>{rooms.map((room) => <option key={room.id} value={room.id} disabled={room.status === "LIVE" || room.status === "CLOSED"}>{room.name} ({room.status})</option>)}</select></td></tr>)}</tbody></table></div>{!loading && !ideas.length ? <div className="px-6 py-12 text-center text-sm text-ink-muted">Nenhuma ideia ativa cadastrada neste ideathon.</div> : null}</section>
      </div>
    </AppShell>
  );
}
